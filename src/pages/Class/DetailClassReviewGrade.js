...
const DetailClassReviewGrade = () => {
  ...
  const API_URL_STUDENT_GRADE =
    ...
    "?review-id=" +
    review_id;
  const API_URL_COMMENT =
    API_URL + "classroom/grade/student/" + id + "/" + grade_id + "/comment";
  const API_URL_SET_FINAL_GRADE =
    API_URL +
    "classroom/grade/student/" +
    id +
    "/" +
  ...
  const handleClickPostComment = () => {
    ...
    axios.post(API_URL_COMMENT, comment, { headers }).then((response) => {
      if (response.status) {
  ...
  const handleClickFinalizeGrade = () => {
    ...
    axios
      .put(API_URL_SET_FINAL_GRADE, finalGrade, { headers })
      .then((response) => {
        if (response.status) {
  ...
  useEffect(() => {
    ...
    axios.get(API_URL_STUDENT_GRADE, { headers }).then((response) => {
      setLoading(false);
        ...
        {loading ? (
          ...
          <div className="card-container">
            ...
            <div className="social-container">
              <div className="gradeDetail">
                ...
                <h2 className="smaller-text">
                  {status ? <CheckCircleOutlineIcon /> : <PendingActionsIcon />}
                </h2>
              </div>
              {finalGrade ? (
                <div className="gradeDetail">
            ...
            <Paper style={{ padding: "40px 20px" }}>
                ...
                <Grid item>
                  ...
                  >
                    <Avatar
                      ...
                      src={
                        classroom.isCustom
                          ? classroom.ownerAvatar
                          : student.avatarUrl
                      }
                      alt={
                        classroom.isCustom ? classroom.ownerName : student.name
                      }
                    />
                  </Box>
                </Grid>
              ...
              {classroom.isCustom && !status && (
                <Grid container wrap="nowrap" spacing={2} id="finalize">
                  <Grid item>
                    <Box
                      sx={{
                        width: 80,
                  ...
                  <Grid item xs zeroMinWidth>
                    <TextField
                      type="number"
                      value={finalizeGrade}
                      onChange={(e) => handleChangeFinalizeGrade(e)}
                    />
                    <Button
                      variant="contained"
                      id="post-button"
                      onClick={handleClickFinalizeGrade}
                    >
                      Finalize
                    </Button>
                  </Grid>
