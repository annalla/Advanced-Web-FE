...
const DetailStudentClass = () => {
  ...
  const handleSubmit = () => {
    ...
    // id is classroom id
    axios
      .post(API_URL_STUDENT_GRADE + id + "/" + selectedGradeId, reviewRequest, {
        headers,
      })
      .then((response) => {
        ...
        if (response.data.status) {
          setNotifyContent(
            "You have successfully submitted an appeal for this score!"
          );
        } else {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(API_URL_STUDENT_GRADE + id, { headers }).then((response) => {
      setLoading(false);
      ...
      data.gradeArray.length
        ? setSelectedGradeId(data.gradeArray[0].id)
    ...
    return (
      <Fragment>
        ...
        <div className="card-container">
          <Box
            sx={{ display: "flex", width: "100%", justifyContent: "center",pt:3 }}
          >
            <Avatar
              src={studentInformation.avatarUrl}
              alt={studentInformation.name}
              sx={{ width: 60, height: 60 }}
            />
          </Box>
          <h1 className="bold-text">
            {studentInformation.name}{" "}
            <span className="normal-text">{studentInformation.code}</span>
          </h1>
          ...
          <div className="social-container">
            {studentGrade.length !== 0
              ? studentGrade.map((grade) => {
                  return (
                    <div className="gradeDetail" key={grade.id}>
                      <h1 className="bold-text">
                        {grade.point}/{grade.maxPoint}
                      </h1>
          ...
          <h1 className="totalGrade">
            {studentInformation.totalGrade}/{studentInformation.maxTotalGrade}
          </h1>
          <h2 className="smaller-text">
            Not satisfied with your score? Submit a review request
            <span> </span>
            <span id="link" onClick={handleClickOpen}>
              here.
            </span>
          </h2>
        </div>
          ...
          {selectedGradeId ? (
            <DialogContent>
              <DialogContentText id="note">
                To submit a request for a review to the teacher of this subject,
        ...
        >
          ...
          <DialogContent>
            <DialogContentText>{notifyContent}</DialogContentText>
          </DialogContent>
          <DialogActions>
